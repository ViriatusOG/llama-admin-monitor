...
### Added
- The Monitor page's runtime strip shows the OpenAI-compatible API URL (`http://<monitor>:<port>/v1`, proxied to the running llama-server) with a Copy button, so the endpoint is discoverable in-app rather than only in the README.
