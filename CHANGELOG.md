...
- **Preset editor: validation and change markers.** Saving now range-checks numeric fields (temperature 0–2, top-p/min-p/draft-p 0–1, top-k/layers/threads ≥ 0, cache MiB ≥ −1, image token minimum ≤ maximum, …) and marks offenders with a red border plus a toast. While the editor is open, every section whose fields differ from the loaded preset grows an accent dot on its header, so you can see what you have changed at a glance.
- **Chat: per-response stats and a Stop button.** Each assistant reply gets a stats line from llama.cpp's final chunk — generation tokens/s, token count, prompt tokens/s, draft acceptance for speculative decoding, and total tokens (requested via `stream_options.include_usage`). The Stop button aborts an in-flight stream; a stopped reply is marked as such.
- **Models page search box** — filters the list by model name, file name or quantization, with an N-of-M note and a no-match state.
- **Log search** — on the Logs page (the monitor's own event log) and on Monitor's Process Output panel, where matching server lines are coloured by level (red for errors, amber for warnings).

### Fixed
- `--cram`/`--cache-ram` was documented in the preset editor as "KV cache compression bits per element"; in llama.cpp it is the **maximum cache size in MiB** (0 = disabled, −1 = no limit, default 8192). The preset field now accepts negative values and the label, help text and docs are corrected.
- `--fitt` was described as "target memory per GPU"; it is the **per-device margin left free (MiB)** when auto-fitting with `--fit`. Label and help text corrected.

### Added
- **Preset editor: Sampling, Reasoning, Memory/KV and projector placement sections.** Sampling (server-level defaults; API clients can still override per request): temperature, top-p, top-k, min-p, repeat/presence/frequency penalties, ignore-eos. Reasoning for hybrid thinking models: mode (`--rea` on/off/auto), thinking effort (`--reasoning-effort`), preserve thinking (`--reasoning-preserve`), chat-template kwargs. Memory/KV: auto-fit context to available memory (`--fit`/`--fitt`), KV cache compression (`--cram`), KV offload to CPU (`--kv-offload`), context shift (`--context-shift`), and a warmup toggle (the `--no-warmup` default is now explicit). Projector: device (`--mmdev`), CPU offload (`--mmproj-offload`), per-image token limits. Speculative decoding gained a spec-type selector (ngram variants, `draft-eagle3`, `draft-mtp`, … — the ngram-spec checkbox still works as before when it is empty) and draft-model tuning (GPU layers, device, threads, batch threads, min probability). All new fields are optional: older presets and llama-server builds are unaffected unless a field is set.
- **DeepSeek page** (sidebar → Interact → DeepSeek): [DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness) (`dsh`) run and embedded from the dashboard. dsh's web UI listens on the server's loopback only and refuses other bind addresses, so the monitor runs `dsh web` in a PTY (its log shows in a collapsible terminal), forwards the monitor's port + 1 (e.g. 7779) to it with a plain TCP proxy, passes matching `--trusted-host` authorities for dsh's browser-trust check, scrapes the login URL (with its one-time token) that dsh prints at startup and shows it with Open / Copy URL / Copy token buttons — the UI runs in its own tab; dsh's output sits in a collapsed section underneath (expanded while installing or starting, or after a failure). Before every start it merges a `llama-admin-monitor` provider into `~/.dsh/settings.yaml` (`llm-pi-ai.providers`) with one model per preset via the monitor's `/v1` proxy, including the compat switches llama-server needs (`supportsDeveloperRole: false`, `maxTokensField: max_tokens`). **Install dsh** runs `npm install -g @deepseek-ai/dsh` in the terminal (Node.js 22+; the Pi installer provides one). `GET /api/dsh/status`, `POST /api/dsh/start|stop|install|update`, `/ws/dsh`.
