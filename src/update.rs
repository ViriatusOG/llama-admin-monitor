...
pub fn apply_update(branch: String) {
    ...
    thread::spawn(move || {
        ...
        #[cfg(unix)]
        {
            ...
            let script = format!(
                r#"
                echo "[info] Fetching latest release info for branch {branch}..."
                if [ "{branch}" = "main" ]; then
                    LATEST_TAG=$(curl -s https://api.github.com/repos/ViriatusOG/llama-admin-monitor/releases/latest | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
                else
                    LATEST_TAG=$(curl -s https://api.github.com/repos/ViriatusOG/llama-admin-monitor/releases | grep '"tag_name":' | head -n 1 | sed -E 's/.*"([^"]+)".*/\1/')
                fi
