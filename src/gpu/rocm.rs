...
/// The best name available for one rocm-smi card entry (see
/// `amd_names::resolve_amd_name`). `None` when rocm-smi gave no name at
/// all, so the caller can use its own key (card0...).
fn amd_display_name(card: &serde_json::Value) -> Option<String> {
    ...
    let text = |key: &str| card.get(key).and_then(|v| v.as_str()).map(str::trim);
    let bus = text("PCI Bus");
    let device = text("Card Model").and_then(parse_hex_id);
    let rev = text("Device Rev").and_then(parse_hex_id);
    let gfx = text("GFX Version").unwrap_or("");
    Some(resolve_amd_name(series, gfx, device, rev, bus))
}
